...
client.on("ready", async () => {
  console.log(`ready!`);
  client.user.setActivity(`Use =help For Help`, { type: "PLAYING" }).catch(error => console.log(error));
});
//hi
...
client.on("message", async message => {
  ...
  if (command) {
    if (!message.guild.me.hasPermission("ADMINISTRATION"))
...
client.on("guildMemberAdd", async member => {
  ...
  let Msg = Message.toLowerCase()
    ...
    .replace("<membermention>", `<@${member.user.id}>`);
  ...
  let Image = await Welcomed.setUsername(member.user.username)
    ...
    .setBackground(
      WelcomeImage || "https://images.wallpaperscraft.com/image/landscape_art_road_127350_1280x720.jpg"
    )
    .toAttachment();
...
client.on("guildMemberRemove", async member => {
  ...
  let Image = await Leaved.setUsername(member.user.username)
    ...
    .setMemberCount(member.guild.memberCount)
    .setBackground(
      LeaveImage || "https://images.wallpaperscraft.com/image/cat_night_lights_74375_1280x720.jpg"
    )
    .toAttachment();
